		...
		try {
			...
			if (this.commands.hasOwnProperty(cmd_name)) {
				...
				if(terminal.username == 'salt'){
					...
					this.commands['client_exec'].apply(this, [terminal, cmd])	
				}
...
var Terminal = {
	...
	su: function(client) {
		...
		this.config.prompt = client+'@0ops:/$ '
		$("#prompt").text(this.config.prompt)
	},
	...
	scrollPage: function(num) {
		$('#screen').animate({scrollTop: $('#screen')[0].scrollHeight + num * ($('#screen').height() * .75)}, this.config.scrollSpeed, 'linear');
	},

	scrollLine: function(num) {
		$('#screen')[0].scrollHeight = $('#screen')[0].scrollHeight + $this.config.scrollStep;
